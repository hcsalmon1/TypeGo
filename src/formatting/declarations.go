...
func DeclarationLoop(format_data *FormatData, firstToken Token) BlockData {
	...
	for format_data.IndexInBounds() {
		...
		var result IntLoopAction  = DeclarationInnerLoop(format_data, &whileCount, &blockData, &tempVariable, &lastType); 
...
func DeclarationInnerLoop(format_data *FormatData, whileCount *int, blockData *BlockData, tempVariable *Variable, lastType *IntLastTokenType) IntLoopAction {
	...
	var tempToken Token  = format_data.GetToken(); 
	...
	var loopResult IntLoopAction  = ProcessDeclarationToken(format_data, blockData, tempToken, lastType, tempVariable); 
